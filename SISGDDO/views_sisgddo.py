...
import psycopg2
from django.db.models import Max, F, Count, Subquery, OuterRef
...
class DashboardGraficobarra(LoginRequiredMixin,TemplateView):
    ...
    def get_grafico_espinform(self):
        ...
        year = datetime.now().year
        for m in range(1,13):
            cantidad_proyectos_iniciados= 0
...
def cargar_datos(request):
    ...
    # Obtener todos los objetivos ordenados por fecha_definicion en orden descendente
    objetivos_ordenados = xavi_models.objetivo.objects.filter(activo=True).order_by('-fecha_definicion')

    # Obtener el año de la fecha más reciente
    if objetivos_ordenados.exists():
        ano_mas_reciente = objetivos_ordenados.first().fecha_definicion.year

        # Filtrar los objetivos que tienen el año de la fecha más reciente
        objetivos_del_ano = xavi_models.objetivo.objects.filter(
            activo=True,
            fecha_definicion__year=ano_mas_reciente
        )

        # Subquery para obtener el periodo con el valor más alto para cada objetivo
        max_periodo_subquery = xavi_models.evaluacion_trimestral.objects.filter(
            objetivo=OuterRef('pk')
        ).order_by('-periodo__orden').values('periodo__orden')[:1]

        

        # Obtener la última evaluación de cada objetivo basado en el periodo con el valor más alto
        evaluaciones = xavi_models.evaluacion_trimestral.objects.filter(
            objetivo__in=objetivos_del_ano,
            periodo__orden=Subquery(max_periodo_subquery)
        ...
        for evaluacion in evaluaciones:
            estado = evaluacion.objetivo.estado
            contador_por_estado[estado] += 1

        total_objetivos = len(objetivos_del_ano)

        contador_por_estado_formato = [
            {
                'name': str(estado.nombre),
                'y': (cantidad / total_objetivos) * 100,
                'drilldown': str(estado.nombre)
            }
            for estado, cantidad in contador_por_estado.items()
        ]

        for item in contador_por_estado_formato:
            print(f"Estado: {item['name']}, Porcentaje de Objetivos: {item['y']:.2f}%")

        datos['objetivos_porciento'] = contador_por_estado_formato
...
def create_afectation(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            estado=models.estados.objects.get(nombre="En proceso", activo=True)
...
def paso_afectacion(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            afectacion.save()
            inc.estado='En afectación a la producción'
            inc.save()
...
def detalle_afectacion(request, numero):
    ...
    equip=objeto.observacionesactual.split('.',-1)    
...
def crear_incidencias(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            inc.save()
            inc.ejecutante.set(form.cleaned_data['ejecutante'])
            inc.estado=estado
            inc.save()
...
def detalle_incidencias(request, id):
    ...
    comentarios=objeto.respuesta.split('.',-1)
    ejecutantes=objeto.ejecutante.all()

    template_name = 'P00/Incidencias/DetalleIncidencias.html'
    contexto = {
        'object' : objeto, 
        'comentarios': comentarios,
        'ejecutantes': ejecutantes,   
    }
    return render(request, template_name, contexto)
...
def cambiar_incidencias(request, id):
    inc = models.incidencia.objects.get(pk = id)
    form = ModifiyIncidenciaModelForm(instance=inc)

    if request.method == 'POST':
        form = ModifiyIncidenciaModelForm(instance=inc, data=request.POST)

        if form.is_valid():
            # Capturamos la respuesta actual antes de cualquier modificación
            respuesta_anterior = inc.respuesta
            nueva_observacion = form.cleaned_data['observacion']
            fecha_actual = datetime.now().date()

            # Guardamos los cambios del formulario pero sin confirmar aún en la base de datos
            inc.estado= form.cleaned_data['estado']
            # Concatenamos la respuesta anterior con la nueva observación y la fecha
            inc.respuesta = f"{respuesta_anterior}  {fecha_actual}  {nueva_observacion}"
            
            # Actualizamos la observación
            inc.save()
            # Si el estado es "Cerrado", actualizamos la fecha de cierre
            if inc.estado == 'Cerrado':
                inc.fecha_cierre = fecha_actual

            # Guardamos todos los cambios en la base de datos
            inc.save()
            logs(request, incidencia, inc, 2)
            return JsonResponse({'results': {'url': reverse_lazy(listar_incidencias)}})
        

    return render(request, 'P00/Incidencias/CambiarIncidencias.html', {'instance': inc, 'form': form})
...
def listar_estados(request):
    contexto = {
        'lista': models.estados.objects.filter(activo = True)
    }
    # print(contexto['lista'])
    return render(request, 'nomencladores/estados/estados.html', contexto)

def agregar_estado(request):
    form = EstadosModelForm()

    if request.method == 'POST':
        form = EstadosModelForm(request.POST)

        if form.is_valid():
            # Crear una nueva instancia del modelo Estados con los datos del formulario
            ...
            logs(request, models.estados, nuevo_estado, 1)
            
            # Retornar una respuesta JSON indicando el éxito de la operación
            return JsonResponse({'results': {'url': reverse_lazy('listar_estados')}})

    # Renderizar la plantilla con el formulario
    return render(request, 'nomencladores/estados/estados_form.html', {'form': form})
@login_required
@handle_exceptions
def modificar_estado(request, id):
    instance = models.estados.objects.get(id = id)
    form = EstadosModelForm(instance=instance)

    if request.method == 'POST':
        form = EstadosModelForm(instance=instance, data=request.POST)

        if form.is_valid():
            instance = form.save()
            logs(request, models.estados, instance, 2)
            return JsonResponse({'results': {'url': reverse_lazy(listar_estados)}})
        else:
            print(str(form.errors))

    return render(request, 'nomencladores/estados/estados_update_form.html', {'instance': instance, 'form': form})

@login_required
@handle_exceptions
def eliminar_estado(request, id):
    objeto = models.estados.objects.get(id = id)
    template_name = 'nomencladores/estados/estados_confirm_delete.html'
    contexto = {
        'object' : objeto
    }
    if request.method == "POST":
        objeto.activo=False
        objeto.save()
        messages.success(request, "Estado eliminado con éxito")
