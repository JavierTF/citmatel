...
const cambiarIncidencia = function () {
    ...
    const initEvent = function () {
        ...
        $('#id_ejecutante').on('change', function () {
            if ($('#id_ejecutante').val() != '') {
                hiden_error_message($('#id_ejecutante'));
                if($('#id_trabajador').val() == $('#id_ejecutante').val()) { 
                    show_error_message($('#id_ejecutante'), 'El trabajador que reporta la incidencia no puede ser el encargado de la misma');
                    return;
                        }else{  
                            hiden_error_message($('#id_trabajador'));
                    hiden_error_message($('#id_ejecutante'));
                ...
                show_error_message($('#id_ejecutante'), 'Este campo es requerido');
            }
        });
        ...
        $('#id_respuesta').on('change', function () {
            if ($('#id_respuesta').val() != '') {
                hiden_error_message($('#id_respuesta'));
            } else {
                show_error_message($('#id_respuesta'), 'Este campo es requerido');
            }
        });
    ...
    hiden_error_message($('#id_fecha_cierre'));
        ...
        if ($('#select2-id_estado-container').val() == 'Cerrado') {
    $('#select2-id_estado-container').on('change', function () {
        if ($('#id_fecha_cierre').val() != '') {
            if(Date.now() > Date.parse($('#id_fecha_cierre').val())) { 
                show_error_message($('#id_fecha_cierre'), 'La fecha de cierre no puede ser menor que la fecha actual');
                   }else{  
                 hiden_error_message($('#id_fecha_cierre'));
                   }
                let ahora=new Date($('#id_fecha_cierre').val())
                let antes=new Date(Date.now())
            if(antes.getFullYear()+2 <= ahora.getFullYear()) { 
                    show_error_message($('#id_fecha_cierre'), 'La fecha de cierre no puede ser 2 años mayor que la fecha actual');
                           ...
                           if(Date.now() < Date.parse($('#id_fecha_cierre').val())) { 
                            ...
                            return;
                             ...
                             hiden_error_message($('#id_fecha_cierre'));
                               ...
                               let a=new Date($('#id_fecha_cierre').val())
                                let an=new Date(Date.now())
                            if(an.getFullYear()-2 >= a.getFullYear()) { 
                                    show_error_message($('#id_fecha_cierre'), 'La fecha de cierre no puede ser 2 años menor que la fecha actual');
                                    return;
