...
def prueba(request):
    try:
        ...
        #proys = models.area.objects.filter(activo = True).exclude(incidencia__isnull=True)
        proys = sisgddo_models.incidencia.objects.filter(activoasd = True).exclude(ejecutante__isnull=True)
...
def listar_proyecto(request):
    ...
    if request.method == 'POST':
        if request.POST['tipo_filtrado'] == 'fecha_inicio': 
            proyectos_filtrados = models.proyecto.objects.filter(Q(registro_aprobacion__activo = True, registro_aprobacion__fecha_inicio__gte=request.POST['fecha_inicio'], registro_aprobacion__fecha_inicio__lte=request.POST['fecha_fin']))
        elif request.POST['tipo_filtrado'] == 'fecha_terminacion':
            proyectos_filtrados = models.proyecto.objects.filter(Q(registro_aprobacion__activo = True, registro_aprobacion__fecha_terminacion__gte=request.POST['fecha_inicio'], registro_aprobacion__fecha_terminacion__lte=request.POST['fecha_fin']))
            # print('\n---------->>>>', len(proyectos_filtrados), '\n')
        # elif request.POST['tipo_filtrado'] == 'fecha_entrega':
        #     proyectos_filtrados = models.proyecto.objects.filter(Q(registro_aprobacion__activo = True, registro_aprobacion__fecha_inicio__gte=request.POST['fecha_inicio'], registro_aprobacion__fecha_inicio__lte=request.POST['fecha_fin']))
        # elif request.POST['tipo_filtrado'] == 'fecha_cierre':
        #     proyectos_filtrados = models.proyecto.objects.filter(Q(registro_aprobacion__activo = True, registro_aprobacion__fecha_inicio__gte=request.POST['fecha_inicio'], registro_aprobacion__fecha_inicio__lte=request.POST['fecha_fin']))
        else:
            proyectos_filtrados = []        
        ...
        queryset = proyectos_filtrados.annotate(num_entradas_activas=Count('entradas', filter=Q(entradas__activo=True))).filter(id__in=id_list).order_by('-id')
                
        print('QUERYSET', queryset)
        
        lista_queryset = list(queryset.values('id', 'nombre_proyecto', 'num_entradas_activas', 'codigo', 'registro_aprobacion__centro_costo__nombre', 'registro_aprobacion__employee', 'sosi'))
