...
def handle_exceptions(view_func):
    ...
    def wrapped_view(request, *args, **kwargs):
        try:
            ...
            return redirect(reverse('error403') + '?excepcion_info=' + json.dumps(excepcion_info))
            ...
            return redirect(reverse('error404') + '?excepcion_info=' + json.dumps(excepcion_info))
        except Exception as e:
            excepcion_info = {
                'tipo': type(e).__name__,
                'mensaje': textwrap.fill(str(e), width=100),
...
def get_ultimo(modelo=proyecto, attrib='no'):
    try:
        # ultimo = modelo.objects.last()
        ultimo = modelo.objects.filter(activo = True).order_by('id').last()
...
def get_ultimo_dependencia(modelo, dependencia):
    try:
        ultimo = modelo.objects.filter(indicador = dependencia).order_by('id').last()
        no = None
