...
from xavi.models import acuerdo, estado_acuerdo, objetivo, evaluacion, proyecto, estado_proyecto
import calendar
...
def error404(request):
    ...
    if excepcion_info_str:
        excepcion_info = json.loads(excepcion_info_str)
        # print(excepcion_info)
    return render(request, "Security/error.html", { 'error_code': 404, 'excepcion_info': excepcion_info})
...
def error500(request):
    ...
    if excepcion_info_str:
        excepcion_info = json.loads(excepcion_info_str)
        # print(excepcion_info)
    return render(request, "Security/error.html", { 'error_code': 500, 'excepcion_info': excepcion_info})
...
def error403(request):
    ...
    if excepcion_info_str:
        excepcion_info = json.loads(excepcion_info_str)
        # print(excepcion_info)
    return render(request, "Security/error.html", { 'error_code': 403, 'excepcion_info': excepcion_info})
...
def db_save(request):
    ...
    if request.method == 'POST':
        ...
        address = f"/home/access/mezclando/media/db/{datetime.now().strftime('%Y%m%d%H%M%S')}_SISGEPO.sql"
...
def db_restore(request):
    if request.method == "POST":
        ...
        try:
            address = "/home/access/mezclando/media/db/" + name
            address = address[:-1]
