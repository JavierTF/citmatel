...
def nomproyecto(request):        
    ...
    if request.method == "POST":
        ...
        ultimo_numero_proyecto = models.proyecto.objects.filter(activo = True).last()
            ...
            try:
                with transaction.atomic():
                    ...
                    try:
                        ...
                        response = {
                            'result': 'errorField',
                            'title': 'Error al crear Jefe de proyecto',
                            'text': '',
                            'error_message': ''
                        }
                        return JsonResponse(response)
                    ...
                    try:
                        ...
                        response = {
                            'result': 'errorField',
                            'title': 'Error al crear Especialista comercial',
                            'text': '',
                            'error_message': ''
                        }
                        return JsonResponse(response)
...
def prueba(request):
    try:
        us = User.objects.get(username = 'marta')
        # us.password('')
        # tr.update()
        #
        # employee = persona_jefa,
        ...
        # return JsonResponse({'ok': str(last.id) })
...
def nomacuerdo(request):
    ...
    if request.method == "POST":
        ...
        if res['result'] == 'errorField' or res['result'] == 'error':
            res['result'] = 'error'
            res['title'] = 'Error en el campo nombre. Ya existe ese nombre'
            res['field'] = 'nombre'
            return JsonResponse(res)
...
def detalle_aprobacion(request, id):
    ...
    rol_jefe = Position.objects.get(active = True, name__iexact = 'Jefe de proyecto')
    myworker = trabajador_registro.objects.get(
        registro = objeto.registro_aprobacion, 
        rol = rol_jefe,
        activo = True
    ).employee
    
    rol_comercial = Position.objects.get(active = True, name__iexact = 'Comercial')
    mycomercial = trabajador_registro.objects.get(
        registro = objeto.registro_aprobacion, 
        rol = rol_comercial,
        activo = True
    ).employee
...
def listar_proyecto(request):
    if request.method == 'GET':
        ...
        jefe_proyecto_subquery = models.trabajador_registro.objects.filter(
            registro=OuterRef('registro_aprobacion'),
        ...
        comercial_subquery = models.trabajador_registro.objects.filter(
            registro=OuterRef('registro_aprobacion'),
        ...
        datos = models.proyecto.objects.annotate(
        ...
        ).filter(activo=True).order_by('-id')
            ...
            try:
                ...
                main_doc = HTML(string=html_string, base_url=request.build_absolute_uri()).render(
                    stylesheets=[
                        ...
                        CSS(string=".pdf-page { height: 100vh; } @page { size: landscape; }"),  # Cambiar a orientación horizontal
...
@login_required
@user_has_any_permission('xavi.delete_proyecto', 'xavi.delete_registro_aprobacion')
@handle_exceptions
def eliminar_aprobacion(request, id):
    try:
        objeto = models.proyecto.objects.get(pk = id).registro_aprobacion
    except ObjectDoesNotExist:
        raise ObjectDoesNotExist
    except:
        raise
    
    template_name = 'P01/proyecto/aprobacion_confirm_delete.html'
    contexto = {
        'object' : objeto
    }
    ...
    if request.method == "POST":
        with transaction.atomic():
            try:
                ...
                response = {}
                response['result'] = 'success'
                response['title'] = 'Registro de aprobación eliminado con éxito'
                return JsonResponse(response)
            except Exception as e:
                print(f"Error durante la eliminación: {e}")
                # messages.error(request, "Error durante la eliminación del Proyecto y ")
