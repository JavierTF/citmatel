...
from SISGDDO.models import incidencia, auditoria_externa, auditoria_interna, ActividadPlan, Contacto, estados
...
class ModifyAfectationModelForm(forms.ModelForm):
    ...
    estado= forms.ModelChoiceField(label="Estado", queryset=estados.objects.filter(activo=True), widget = widgets.Select(attrs = {'class': 'form-control texto select2','autocomplete': 'on'}))
    ...
    class Meta:
       ...
       'afectacion':widgets.Textarea(attrs={'rows':1, 'class': 'form-control'}),
        'fecha_recepcion': widgets.DateInput(attrs={'type': 'date', 'class': 'form-control','append': 'fa fa-calendar', 'icon_toggle': True}), 
        'observacionesactual': widgets.Textarea(attrs={'rows':3, 'class': 'form-control'}),
        'estado': widgets.Select(attrs={'class': 'form-control texto select2', 'autocomplete': 'on'}),
...
#moduloIncidencia
class EstadosModelForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super(EstadosModelForm, self).__init__(*args, **kwargs)
...
class IncidenciaModelForm(forms.ModelForm):
    ...
    ejecutante = forms.ModelMultipleChoiceField(queryset = area.objects.all().exclude(activo=False).order_by('nombre'), required = True, label = 'Ejecutor', widget = widgets.SelectMultiple(attrs = {'class': 'form-control texto select2','autocomplete': 'on'}))
    hora = forms.TimeField(label="Fecha C", initial=datetime.now),
    descripcion = forms.CharField(label="descrip", max_length=1000),
    respuesta = forms.CharField(label="Seguimiento", max_length=1000),
    ...
    class Meta:
        ...
        widgets = {
            ...
            'fecha_cierre': widgets.DateInput(attrs={'type': 'date', 'class': 'form-control', 'append': 'fa fa-calendar', 'icon_toggle': True}),
            'descripcion': widgets.Textarea(attrs={'rows': 7, 'class': 'form-control'}),
...
class ModifiyIncidenciaModelForm(forms.ModelForm):
    ...
    class Meta:
        model = incidencia
        fields = "estado","respuesta","observacion"
        widgets = {
        'titulo':widgets.Textarea(attrs={'rows':1, 'class': 'form-control'}),
        'fecha_recepcion': widgets.DateInput(attrs={'type': 'date', 'class': 'form-control','append': 'fa fa-calendar', 'icon_toggle': True}), 
        'fecha_cierre': widgets.DateInput(attrs={'type': 'date', 'class': 'form-control','append': 'fa fa-calendar', 'icon_toggle': True}),
